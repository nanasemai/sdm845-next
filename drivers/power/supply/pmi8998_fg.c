...
#define IMA_EXP_BITS	(IACS_ERR_BIT | XCT_TYPE_ERR_BIT | DATA_RD_ERR_BIT | \
			 DATA_WR_ERR_BIT | ADDR_BURST_WRAP_BIT | ADDR_STABLE_ERR_BIT)

#define WRITE_SEC_ACCESS_OFFSET		0xd0
#define WRITE_SEC_ACCESS_VALUE		0xa5
...
struct pmi8998_fg_chip {
	...
	/* ensure secure access registers writes */
	spinlock_t secure_access_lock;
	int status;
...
/**
 * pmi8998_fg_read() - Read multiple registers with regmap_bulk_read
 *
 * @chip: Pointer to chip
 * @val: Pointer to read values into
...
{
	u16 base = chip->base + addr;

	if (base & ~PERIPHERAL_MASK == 0)
		return -EINVAL;

	return regmap_bulk_read(chip->regmap, base, val, len);
}

/**
 * pmi8998_fg_write() - Write multiple registers with regmap_bulk_write
 *
 * @chip: Pointer to chip
 * @val: Pointer to write values from
 * @addr: Address to write to
 * @len: Number of registers (bytes) to write
 * Return:
 * %0 - OK
 * %-EINVAL - invalid argument
 */
...
{
	bool secure_access = (addr & PERIPHERAL_MASK) > WRITE_SEC_ACCESS_OFFSET;
	u16 base = (chip->base + addr);
	u8 secure_addr_val = WRITE_SEC_ACCESS_VALUE;
	unsigned long flags;
	int ret;
	...
	if (secure_access) {
		spin_lock_irqsave(&chip->secure_access_lock, flags);
		ret = regmap_bulk_write(chip->regmap,
					base & ~PERIPHERAL_MASK |
					WRITE_SEC_ACCESS_OFFSET,
					&secure_addr_val, 1);
		if (ret)
			goto out;
	}

	ret = regmap_bulk_write(chip->regmap, base, val, len);

out:
	if (secure_access)
		spin_unlock_irqrestore(&chip->secure_access_lock, flags);

	return ret;
}

/**
 * pmi8998_fg_update() - Update register
 *
 * @chip: Pointer to chip
 * @val: Pointer to write values from
 * @addr: Address to write to
 * @mask: Mask which has to be applied
 *
 * Return:
 * %0 - OK
 */
static int pmi8998_fg_update(struct pmi8998_fg_chip *chip, u16 addr, u8 mask, u8 val)
{
	bool secure_access = (addr & PERIPHERAL_MASK) > WRITE_SEC_ACCESS_OFFSET;
	u16 base = chip->base + addr;
	u8 secure_addr_val = WRITE_SEC_ACCESS_VALUE;
	...
	ret = regmap_update_bits(chip->regmap, base, mask, val);
...
/**
 * pmi8998_fg_get_capacity() - Get remaining capacity of battery
 *
 * @chip: Pointer to chip
 * @val: Pointer to store value at
 *
 * Return:
 * %0 - OK
 */
...
/**
 * pmi8998_fg_get_temperature() - Get temperature of battery
 *
 * @chip: Pointer to chip
 * @val: Pointer to store value at
 *
...
/**
 * pmi8998_fg_get_current() - Get current being drawn from battery
 *
 * @chip: Pointer to chip
 * @val: Pointer to store value at
 *
...
/**
 * pmi8998_fg_get_voltage() - Get voltage of battery
 *
 * @chip: Pointer to chip
 * @val: Pointer to store value at
 *
...
/**
 * pmi8998_fg_get_temp_threshold() - Get configured temperature thresholds
 *
 * @chip: Pointer to chip
 * @psp: Power supply property of temperature limit
 * @val: Pointer to store value at
 *
 * Return:
 * %0 - OK
				   ...
				   union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		return pmi8998_fg_get_capacity(chip, &val->intval);
		break;
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		return pmi8998_fg_get_current(chip, &val->intval);
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		return pmi8998_fg_get_voltage(chip, &val->intval);
		break;
	case POWER_SUPPLY_PROP_PRESENT:
		val->intval = 1;
		break;
	case POWER_SUPPLY_PROP_TEMP:
		return pmi8998_fg_get_temperature(chip, &val->intval);
		break;
	case POWER_SUPPLY_PROP_TEMP_MIN:
	case POWER_SUPPLY_PROP_TEMP_MAX:
	case POWER_SUPPLY_PROP_TEMP_ALERT_MIN:
	case POWER_SUPPLY_PROP_TEMP_ALERT_MAX:
		return pmi8998_fg_get_temp_threshold(chip, psp, &val->intval);
		break;
	default:
...
static int pmi8998_fg_iacs_clear_sequence(struct pmi8998_fg_chip *chip)
{
	...
	/* clear the error */
	ret = pmi8998_fg_update(chip, MEM_INTF_IMA_CFG, BIT(2), BIT(2));
	if (ret) {
	...
	ret = pmi8998_fg_update(chip, MEM_INTF_IMA_CFG, BIT(2), 0);
	if (ret)
		dev_err(chip->dev, "Failed to write IMA_CFG: %d\n", ret);
...
static int pmi8998_fg_clear_ima(struct pmi8998_fg_chip *chip, bool check_hw_sts)
{
	...
	if (exp_sts & IMA_EXP_BITS) {
		dev_dbg(chip->dev, "IMA exception bits set, exp_sts=%x\n", exp_sts);
		run_err_clr_seq = true;
	}
...
static int pmi8998_fg_clear_dma(struct pmi8998_fg_chip *chip)
{
	u8 dma_status;
	...
	error_present = dma_status & (DMA_WRITE_ERROR_BIT | DMA_READ_ERROR_BIT);
	ret = pmi8998_fg_update(chip, MEM_IF_DMA_CTL, DMA_CLEAR_LOG_BIT,
...
static int pmi8998_fg_probe(struct platform_device *pdev)
{
	...
	if (!prop_addr)
		dev_err_probe(chip->dev, -EINVAL,
			      "Failed to read SOC base address from dt\n");
	 ...
	 */
	ret = pmi8998_fg_update(chip, MEM_INTF_IMA_CFG, BIT(3), BIT(3));
	if (ret)
	...
	if (ret && ret != -EAGAIN)
		dev_err_probe(chip->dev, ret,
			      "Failed to clear IMA exception: %d\n", ret);

	ret = pmi8998_fg_clear_dma(chip);
	if (ret && ret != -EAGAIN)
		dev_err_probe(chip->dev, ret,
			      "Failed to clear DMA exception: %d\n", ret);
	...
	if (IS_ERR(chip->batt_psy))
		return dev_err_probe(&pdev->dev, PTR_ERR(chip->batt_psy,
				     "Failed to register battery\n");

	platform_set_drvdata(pdev, chip);

	/* Get msoc-delta IRQ */
	irq = of_irq_get_byname(pdev->dev.of_node, "msoc-delta");
	if (irq < 0)
		dev_err_probe(&pdev->dev, irq,
			      "Failed to get irq msoc-delta byname: %d\n", irq);

	ret = devm_request_threaded_irq(chip->dev, irq, NULL,
					pmi8998_fg_handle_soc_delta,
					IRQF_ONESHOT, "msoc-delta", chip);
	if (ret < 0)
		dev_err_probe(&pdev->dev, ret,
			      "Failed to request msoc-delta IRQ: %d\n", ret);

	spin_lock_init(&chip->secure_access_lock);
	...
	if (IS_ERR(chip->chg_psy)) {
		dev_warn(chip->dev, "Failed to get optional charger supply: %d\n",
