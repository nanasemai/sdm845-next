...
static int tfa98xx_of_get_named_gpio(struct device_node *np, const char *con_id)
{
	struct gpio_desc *desc;
	int gpio;

	desc = fwnode_gpiod_get_index(of_fwnode_handle(np), con_id, 0,
				      GPIOD_ASIS, con_id);
	if (IS_ERR(desc))
		return PTR_ERR(desc);
			    ...
			    struct device_node *np)
{
	u32 value;
	int ret;
	tfa98xx->reset_gpio = tfa98xx_of_get_named_gpio(np, "reset");
	if (tfa98xx->reset_gpio < 0)
		dev_dbg(dev,
			"No reset GPIO provided, will not HW reset device\n");

	tfa98xx->irq_gpio = tfa98xx_of_get_named_gpio(np, "irq");
...
static struct of_device_id tfa98xx_dt_match[] = {
	{ .compatible = "nxp,tfa9894" }, { .compatible = "tfa,tfa98xx" },
	{ .compatible = "tfa,tfa9872" }, { .compatible = "tfa,tfa9873" },
	{ .compatible = "tfa,tfa9875" }, { .compatible = "tfa,tfa9874" },
